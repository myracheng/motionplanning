...
class DubinsEnv(gym.Env):
    ...
    def __init__(self):
        ...
        plt.ion()
        plt.show()
        self.ax.add_patch(patches.Circle((self.x_g, self.y_g), 1.0, facecolor='g'))
    ...
    def get_measurement(self, s):
        N = 10
        sm = np.zeros((2*N+1,2*N+1))
        dx = 0.4
    ...
    def reset(self, s=np.array([-1,-1,-1])):
        self.arrived = False
        low = np.array([0, 0, -np.pi])
        high = np.array([self.width, self.height, np.pi])
        if (s[0] == -1):
